<?php
defined('YII2_ROOT') or define('YII2_ROOT', __DIR__);
$yii2path = __DIR__ . '/vendor/yiisoft/yii2';
require($yii2path . '/BaseYii.php'); // Yii 2.x

$yii1path = __DIR__ . '/../framework';
...
class Yii extends \yii\BaseYii
{
    ...
    public static function import($alias,$forceInclude=false)
    {
        ...
        if(($pos=strrpos($alias,'\\'))!==false) // a class name in PHP 5.3 namespace format
        {
            ...
            {   if($alias == 'application.models.*') {var_dump($path);exit;}
        ...
        else
        {
            ...
            if(strpos($className,'\\')===false)  // class without namespace
            {
                if(self::$enableIncludePath===false)
                {
                    ...
                    //smarty_internal开头的类不使用Yii autoload 进行加载
                    if(strpos(strtolower($className), 'smarty_internal') === 0){
                        return false;
                    }
                    else
            ...
            {   
                ...
                {
                    $yii2_loader = require(__DIR__ . '/vendor/autoload.php');
                    $result = $yii2_loader->loadClass($className);
                    $yii2_loader->unregister();
     ...
     * @param string $route controller/action 的路由
     */
    public static function load_yii2($moduleId, $route){
        
        /*$request = self::$app->getRequest();
        list(, $params) = $request->resolve();
        unset($params['_m'], $params['_c'], $params['_a']);
        $module = self::$app->getModule($moduleId);
        $result = $module->runAction($route, $params);
    ...
    {    
        ...
        $getValue = function ($key, $array){
            if(! isset($array[$key]) ){
                throw new \CException(Yii::t('application', 'Params Error!'));
            }
