...
function xhprof_generate_image_by_dot($dot_script, $type) {
  ...
  $output_file = ini_get('xhprof.output_dir'). '/xhprof.png';
  
  $temp_file = tempnam(ini_get('xhprof.output_dir'), 'xhprof');
  file_put_contents($temp_file, $dot_script);
       
  $cmd = " c:\\Graphviz2.30\\bin\\dot.exe -T".$type. " $temp_file -o". $output_file;
  
  exec($cmd);

  //$process = proc_open($cmd, $descriptorspec, $pipes, "c:", array());
  unlink($temp_file);
